...
create or replace package body  pacote-funcoes is
	...
	RETURN VARCHAR IS 
	BEGIN RETURN ( Função multiplicar )
	END MULTIPLICARR;
	
	FUNCTION RETORNAVALORDAPOTENCIAE(BASE IN NUMBER, EXPOENTE IN NUMBER)
	RETURN VARCHAR IS
	BEGIN RETURN (‘ Função retorna valor da potencia’ )
	END RETORNAVALORDAPOTENCIAE;
	
	fuction calcularSalario(salalario in number)
	return varchar is
	begin return (Funcao calcular salario)
	end calcularSalario;
...
(nome_tabela IN VARCHAR)
IS 
comando VARCHAR2(100);
teste   VARCHAR2(100);
	BEGIN
	if nome_tabela == HOSPITAL then
		comando := 'create table '|| nome_tabela ||' ( 
			ID INTEGER,
			NAME VARCHAR(100),
			CNPJ INTEGER,
			CONSTRAINT PK_HOSPITAL PRIMARY KEY(ID) )';
	else if nome_tabela == ESPECIALIDADE then
		comando := 'create table '|| nome_tabela ||' (
			ID INTEGER,
			NOME VARCHAR(100),
			DESCRICAO VARCHAR(300),
			CONSTRAINT PK_ESPECIALIDADE PRIMARY KEY(ID));'
	else if nome_tabela == PLANO_SAUDE then
		comando := 'create table '|| nome_tabela ||' ( 	ID INTEGER,
			NOME VARCHAR(100),
			TELEFONE INT,
			ID_CATEGORIA INTEGER,
			CONSTRAINT FK_CATEGORIA FOREIGN KEY(ID_CATEGORIA)REFERENCES CATEGORIA(ID),
			CONSTRAINT PK_SAUDE PRIMARY KEY(ID) )';
	else if nome_tabela == CATEGORIA then
		comando := 'create table '|| nome_tabela ||' ( ID INTEGER,
			NOME VARCHAR(100),
			VALOR INT,
			CONSTRAINT PK_CATEGORIA PRIMARY KEY(ID) )';
	else if nome_tabela == PACIENTE then
		comando := 'create table '|| nome_tabela ||' (ID INTEGER,
			NAME VARCHAR(100),
			CPF INTEGER,
			ID_HOSPITAL INTEGER,
			...
			CONSTRAINT FK_PLANO_PACIENTE FOREIGN KEY(ID_PLANO)REFERENCES PLANO_SAUDE(ID),
			CONSTRAINT PK_PACIENTE PRIMARY KEY(ID))';
	else if nome_tabela == MEDICOS then
		comando := 'create table '|| nome_tabela ||' (ID INTEGER ,
			NOME VARCHAR(100),
			RG INTEGER,
			CRE VARCHAR(50),
			...
			CONSTRAINT FK_HOSPITAL_MEDICOS FOREIGN KEY(ID_HOSPITAL)REFERENCES HOSPITAL(ID),
			CONSTRAINT PK_MEDICOS PRIMARY KEY(ID) )';
	else if nome_tabela == ENFERMEIRO then
		comando := 'create table '|| nome_tabela ||' ( ID INTEGER,
			NOME VARCHAR(100),
			RG INTEGER,
			CRE VARCHAR(50),
			...
			CONSTRAINT FK_HOSPITAL_ENFERMEIRO FOREIGN KEY(ID_HOSPITAL)REFERENCES HOSPITAL(ID),
			CONSTRAINT PK_ENFERMEIRO PRIMARY KEY(ID))';
	else if nome_tabela == ATENDIMENTO then
		comando := 'create table '|| nome_tabela ||' ( ID INTEGER,
			DATA_HORA DATE,
			ID_MEDICO INTEGER,
			ID_PACIENTE INTEGER,
			...
			CONSTRAINT FK_PACIENTE_ATENDIMENTO  FOREIGN KEY(ID_PACIENTE) REFERENCES PACIENTE(ID) ,
			CONSTRAINT PK_ATENDIMENTO PRIMARY KEY(ID) )';
	else if nome_tabela == CREDENCIAMENTO_HOSPITAL then
		comando := 'create table '|| nome_tabela ||' (ID INTEGER,
			DATA DATE,
			ID_HOSPITAL INTEGER,
			ID_PLANO INTEGER,
			...
			CONSTRAINT FK_PLANO_SAUDE_CREDENCIAMENTOH  FOREIGN KEY(ID_PLANO)REFERENCES PLANO_SAUDE(ID),
			CONSTRAINT PK_CREDENCIAMENTO_HOSPITAL PRIMARY KEY(ID) )';
	else if nome_tabela == CREDENCIAMENTO_MEDICO then
		comando := 'create table '|| nome_tabela ||' ( ID INTEGER,
			DATA DATE,
			ID_MEDICO INTEGER,
			ID_PLANO INTEGER,
		...
		comando := 'create table '|| nome_tabela ||' ( ID INTEGER,
			...
			CONSTRAINT FK_ENFERMEIRO_ALAS FOREIGN KEY(ID_ENFERMEIRO) REFERENCES ENFERMEIRO(ID),
			CONSTRAINT PK_ALAS PRIMARY KEY(ID))';
	else if nome_tabela == HISTORICO then
		comando := 'create table '|| nome_tabela ||' ( ID NUMBER NOT NULL,
			ID_PLANO_SAUDE INTEGER,
			ID_CATEGORIA INTEGER,
			CONSTRAINT FK_PLANO_SAUDE_HISTORICO FOREIGN KEY(ID_PLANO_SAUDE)REFERENCES PLANO_SAUDE(ID),
			CONSTRAINT FK_CATEGORIA_HISTORICO FOREIGN KEY(ID_CATEGORIA)REFERENCES CATEGORIA(ID),
			CONSTRAINT HISTORICO_PK PRIMARY KEY (ID) )';
	else if nome_tabela == INTERNACAO then
		comando := 'create table '|| nome_tabela ||' ( ID INTEGER,
			DATA_HORA_ENTRADA DATE,
			DATA_HORA_SAIDA DATE,
			ID_PACIENTE INTEGER,

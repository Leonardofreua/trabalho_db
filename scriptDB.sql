...
CREATE TABLE CREDENCIAMENTI_HOSPITAL(
	ID INTEGER,
	DATA DATE,
	ID_HOSPITAL INTEGER,
	ID_PLANO INTEGER,
	CONSTRAINT FK_HOSPITAL_CREDENCIAMENTIH FOREIGN KEY(ID_HOSPITAL) REFERENCES HOSPITAL(ID),
	CONSTRAINT FK_PLANO_SAUDE_CREDENCIAMENTIH  FOREIGN KEY(ID_PLANO)REFERENCES PLANO_SAUDE(ID),
    CONSTRAINT PK_CREDENCIAMENTI_HOSPITAL PRIMARY KEY(ID)
);
CREATE TABLE CREDENCIAMENTI_MEDICO(
	ID INTEGER,
	DATA DATE,
	ID_MEDICO INTEGER,
	ID_PLANO INTEGER,
	CONSTRAINT FK_MEDICO_CREDENCIAMENTIM FOREIGN KEY(ID_MEDICO) REFERENCES MEDICOS(ID),
	CONSTRAINT FK_PLANO_SAUDE_CREDENCIAMENTIM  FOREIGN KEY(ID_PLANO)REFERENCES PLANO_SAUDE(ID),
    CONSTRAINT PK_CREDENCIAMENTI_MEDICO PRIMARY KEY(ID)
);
...
CREATE TABLE INTERNACAO(
	...
	CONSTRAINT FF_ALAS_INTERNACAO FOREIGN KEY(ID_ALA )REFERENCES ALAS(ID),
    CONSTRAINT PK_INTERNACAO PRIMARY KEY(ID)
);

CREATE TABLE auditoria
(id NUMBER NOT NULL,
CATEGORIA_antigo NUMBER(10,2),
CATEGORIA_novo NUMBER(10,2),
...
CREATE TRIGGER credenciamento_hospital_trig BEFORE INSERT ON CREDENCIAMENTI_HOSPITAL FOR EACH ROW
BEGIN
  :NEW.id := credenciamento_hospital_seq.NEXTVAL;
END;

CREATE TRIGGER credenciamento_medico_trig BEFORE INSERT ON CREDENCIAMENTI_MEDICO FOR EACH ROW
BEGIN
...
create function calcularSalarioo(salalario in number)
return number is
    isento  NUMBER := 0;
    soma NUMBER := 0;
    cont NUMBER :=1;
...
DECLARE
    ...
    CURSOR CURSOR_MEDICOS IS
        ...
        PACIENTE ON PACIENTE.ID = ATENDIMENTO.ID_PACIENTE JOIN CREDENCIAMENTI_MEDICO
        ON MEDICOS.ID = CREDENCIAMENTI_MEDICO.ID_MEDICO JOIN PLANO_SAUDE ON
        PLANO_SAUDE.ID = CREDENCIAMENTI_MEDICO.ID_PLANO WHERE MEDICOS.FORMACAO = 2015 AND
...
CREATE TRIGGER UPDATE_PLANO_SAUDE_CLIENTE
BEFORE UPDATE OF CATEGORIA ON PLANO_SAUDE
BEGIN
    INSERT INTO AUDITORIA (ID, CATEGORIA_ANTIGO, CATEGORIA_NOVO)
    VALUES(:OLD.ID, OLD.CATEGORIA, :NEW.CATEGORIA);
END UPDATE_PLANO_SAUDE_CLIENTE;
